 ...
 * Forked from 10up
 */
if ( ! class_exists( 'Reveal_Slide_Reordering' ) ) :

	class Reveal_Slide_Reordering {
		/**
		 * Handles initializing this class and returning the singleton instance after it's been cached.
		 *
		 * @return null|Simple_slide_Ordering
		 */
		...
		/**
		 * Load up slide ordering scripts for the edit screen
		 */
		public static function load_edit_screen() {
			...
			if ( ! $sortable = apply_filters( 'reveal_slide_reordering_is_sortable', $sortable, $post_type ) ) {
				return;
			}
		...
		public static function ajax_reveal_slide_ordering() {
			...
			do_action( 'reveal_slide_reordering_pre_order_posts', $post, $start );
			...
			$max_sortable_posts = (int) apply_filters( 'reveal_slide_reordering_limit', 50 );    // should reliably be able to do about 50 at a time
			...
			// max per request
			if ( ! isset( $return_data->next ) && $siblings->max_num_slides > 1 ) {
			...
			do_action( 'reveal_slide_reordering_ordered_posts', $post, $new_pos );
		...
		private static function check_edit_others_caps( $post_type ) {
			...
			return apply_filters( 'reveal_slide_reordering_edit_rights', current_user_can( $edit_others_cap ), $post_type );
		}
	}

	Reveal_Slide_Reordering::get_instance();
